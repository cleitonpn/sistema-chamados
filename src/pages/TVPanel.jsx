...
const TVPanel = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeTickets = onSnapshot(query(collection(db, 'chamados')), (snapshot) => {
      ...
      setActivityFeed(sortedTickets.slice(0, 10).map(ticket => {
        ...
        let message, icon;
        // Lógica para mensagem do feed omitida para brevidade
  ...
  return (
    // ✅ LINHA CORRIGIDA COM O FUNDO VERDE-ESCURO
    <div className="min-h-screen h-screen max-h-screen bg-green-900 text-white p-4 flex flex-col gap-4">
      <header className="flex justify-between items-center flex-shrink-0">
        <div><h1 className="text-4xl font-bold text-white">📊 Painel Operacional</h1></div>
        <div className="text-right"><div className="text-4xl font-mono text-white/90">{currentTime.toLocaleTimeString('pt-BR')}</div><div className="text-lg text-white/70">{currentTime.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })}</div></div>
      </header>
      ...
      <div className="flex flex-grow gap-4 min-h-0">
        <main className="flex flex-col flex-grow gap-4 w-3/4">
          ...
          <section className="grid grid-cols-5 gap-4">
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Total</h3><BarChart3 className="h-6 w-6" /></div><div className="text-5xl font-bold">{stats.total}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Abertos</h3><AlertOctagon className="h-6 w-6 text-orange-400" /></div><div className="text-5xl font-bold">{stats.abertos}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Em Andamento</h3><Zap className="h-6 w-6 text-teal-400" /></div><div className="text-5xl font-bold">{stats.emAndamento}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Aguard. Validação</h3><UserCheck className="h-6 w-6 text-yellow-400" /></div><div className="text-5xl font-bold">{awaitingValidationCount}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Concluídos</h3><CheckCircle className="h-6 w-6 text-green-400" /></div><div className="text-5xl font-bold">{stats.concluidos}</div></div>
          </section>

          <section className="grid grid-cols-5 gap-4">
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Projetos Ativos</h3><FolderOpen className="h-6 w-6" /></div><div className="text-5xl font-bold">{projectStats.ativos}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Escalados</h3><TrendingUp className="h-6 w-6 text-indigo-400" /></div><div className="text-5xl font-bold">{escalatedCount}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Aprov. Gerência</h3><GitPullRequest className="h-6 w-6 text-purple-400" /></div><div className="text-5xl font-bold">{pendingApprovalCount}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Abertos Hoje</h3><Calendar className="h-6 w-6" /></div><div className="text-5xl font-bold">{openedToday}</div></div>
            <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><h3 className="text-lg font-bold">Abertos no Mês</h3><Calendar className="h-6 w-6" /></div><div className="text-5xl font-bold">{openedThisMonth}</div></div>
          </section>

          <section className="grid grid-cols-4 gap-4 flex-grow">
              <div className="bg-black/20 border border-red-500 rounded-xl p-4 flex flex-col justify-center">
              ...
              <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col">
                  ...
                  <div className="space-y-2 flex-grow flex flex-col justify-center">
                      {untreatedByArea.length > 0 ? untreatedByArea.map(item => <div key={item.area} className="flex justify-between items-center bg-black/20 p-2 rounded-lg"><span className="font-medium text-yellow-300">{item.area}</span><span className="font-bold text-2xl text-black bg-yellow-400 rounded-md px-2">{item.count}</span></div>) : <p className="text-white/60 text-center py-4">Nenhum chamado aberto!</p>}
                  </div>
              </div>
              <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center">
                  <h3 className="text-lg font-bold mb-2">Taxa de Resolução</h3>
                  <div className="flex items-center justify-between mb-2"><span className="text-5xl font-bold text-green-400">{resolutionRate.toFixed(1)}%</span><span className="text-sm text-white/70">Meta: {RESOLUTION_GOAL}%</span></div>
                  <div className="w-full bg-black/20 rounded-full h-4"><div className="bg-green-500 h-4 rounded-full progress-bar" style={{width: `${resolutionRate}%`}}></div></div>
              </div>
              <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col justify-center">
                  ...
                  <p className="text-white/70 mt-2">Dos chamados necessitam de outras áreas/gerência.</p>
              </div>
          </section>
          ...
          <section className="grid grid-cols-2 gap-4 flex-grow">
              <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col">
                  ...
                  <div className="space-y-2 flex-grow flex flex-col justify-center">
                      {topExecutors.length > 0 ? topExecutors.map(item => <div key={item.name} className="flex items-center bg-black/20 p-2 rounded-lg"><span className="font-medium text-green-300 flex-grow">{item.name}</span><span className="font-bold text-2xl text-black bg-green-400 rounded-md px-3">{item.count}</span></div>) : <p className="text-white/60 text-center py-4">Nenhuma execução este mês.</p>}
                  </div>
              </div>
              <div className="bg-black/20 border border-white/20 rounded-xl p-4 flex flex-col">
                  ...
                  <div className="space-y-2 flex-grow flex flex-col justify-center">
                      {topAreaCreators.length > 0 ? topAreaCreators.map(item => <div key={item.area} className="flex items-center bg-black/20 p-2 rounded-lg"><span className="font-medium text-cyan-300 flex-grow">{item.area}</span><span className="font-bold text-2xl text-black bg-cyan-400 rounded-md px-3">{item.count}</span></div>) : <p className="text-white/60 text-center py-4">Sem dados de criação.</p>}
                  </div>
              </div>
          </section>
        </main>

        <aside className="bg-black/20 border border-white/20 rounded-xl p-4 shadow-lg flex flex-col w-1/4">
